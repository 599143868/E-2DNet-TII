...
Umi = sdpvar(size(Pmax_mi,1),max(FuelType),DGnum,'full');
%% 目标函数
Objective = 0;
for i = 1:DGnum    
    for fuel = 1:FuelType(i)
        for m = 1:K(i,fuel)
            Objective = Objective + (bmi(m,fuel,i)*Pmi(m,fuel,i)+cmi(m,fuel,i)*Umi(m,fuel,i));
        end
    end
end
%% 约束条件
Constraints = [];
for i = 1:DGnum
    Constraints = [Constraints,P(i)==sum(sum(Pmi(1:max(K(i,:)),1:FuelType(i),i)))];
end
for i = 1:DGnum    
    for fuel = 1:FuelType(i)
        for m = 1:K(i,fuel)
            Constraints = [Constraints,Pmin_mi(m,fuel,i)*Umi(m,fuel,i) <= Pmi(m,fuel,i)];
            Constraints = [Constraints,Pmax_mi(m,fuel,i)*Umi(m,fuel,i) >= Pmi(m,fuel,i)];
        end
    end
end
for i = 1:DGnum
    Constraints = [Constraints,sum(sum(Umi(1:max(K(i,:)),1:FuelType(i),i)))==1];
end
for i = 1:DGnum
